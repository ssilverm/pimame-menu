...
class PMList(pygame.sprite.OrderedUpdates):
	...
	def __init__(self, rom_data, cfg, get_labels = False):
		...
		self.back_item = {'type': 'back', 'title': str(platform[1]), 'image_file': '/home/pi/pimame/pimame-menu/assets/images/' + str(platform[0]) + '.png', 'command': None,
							'release_date': platform[3], 'overview': platform[2], 'esrb': '', 'genres': '', 'players':'', 'coop':'', 'publisher': '', 'developer': '', 'rating': '', 'flags':'display_platform'}
		...
		self.rom_template = PMRomItem('', self.cfg, self.cfg.options.rom_list_font, self.cfg.options.rom_list_font_color, self.cfg.options.rom_list_background_color, self.cfg.options.rom_list_font_bold, self.cfg.options.rom_list_offset, create_romlist_image, False, min_scale_size)
		self.selected_rom_template = PMRomItem('', self.cfg, self.cfg.options.rom_list_font, self.cfg.options.rom_list_font_selected_color, self.cfg.options.rom_list_background_selected_color, self.cfg.options.rom_list_font_selected_bold, self.cfg.options.rom_list_offset, create_romlist_selected, False, min_scale_size)
