...
import urllib, urllib2, yaml, zipfile, zlib, os, json, re
import platform
...
class Gamesdb_API(object):
	...
	#Match rom filenames against database real names		
	def match_rom_to_db(self, input_data, min_match_ratio=.6):
		...
		if real_platform['name'].lower() == 'arcade':
			...
			if isfile('/home/pi/pimame/pimame-menu/assets/dat/' + real_platform['shortcode'].lower() + '.dat'):
				console = self.match_rom_to_dat_file(input_data, real_platform['shortcode'].lower() + '.dat')
				return console
				
		#load rom filenames, initialize rom_list to return matches
		print
		print 'Fetching \033[94m%s\033[0m rom list...' % input_data['platform']
		...
		for rom in roms:
			...
			for index, game in enumerate(game_info):
				Lratio = ratio(rom_name, game.ascii_title)
				if Lratio > hi_score:
					hi_score = Lratio
					best_match_game = index
			if best_match_game > -1:
				print 'Best match for \033[95m%s\033[0m is \033[94m%s\033[0m' % (rom_name, game_info[best_match_game].title)
	...
	def get_crc(self, filename): return "%08X"%(zlib.crc32(open(filename,"rb").read()) & 0xFFFFFFFF)
	
	def match_rom_to_dat_file(self, input_data, dat_file, min_match_ratio=.6):
		
		print
		print "Found %s..." % dat_file
		...
		for rom in roms:
			...
			try:
				...
				for info in file.infolist():
					crc = "%08X" % info.CRC
					game = root.find("./game/rom[@crc='" + crc + "']/..")
					if game is not None: break
			except: 
					crc = self.get_crc(os.path.join(input_data['rom_path'],rom))
					game = root.find("./game/rom[@crc='" + crc + "']/..")
			
			if game is not None:
				hi_score = min_match_ratio
				best_match_game = -1
				#rom_name = game name - country tags and extra gibberish
				rom_name = re.sub(r'\([^)]*\)|\[[^)]*\]', '', game.attrib['name']).strip()
				for index, game in enumerate(game_info):
					Lratio = ratio(rom_name, game.ascii_title)
					if Lratio > hi_score:
						hi_score = Lratio
						best_match_game = index
				if best_match_game > -1:
					print 'Database match: \033[95m%s\033[0m -> \033[94m%s\033[0m' % (rom, game_info[best_match_game].title)
					image_file = rom_name + os.path.splitext(game_info[best_match_game].box_art)[1]
					rom_list.append({
						'rom_path': input_data['rom_path'], 
						'image_path': input_data['image_path'], 
	...
	def match_mame_to_dat_file(self, input_data, dat_file):
			...
			for element in game:
				if element.tag == 'rom':
					if not element.attrib['crc'] in crc_list:
						missing_file = True
						print ("%12s - \033[91mcrc doesn't match" + '\033[0m') % rom
						break
				if element.tag == 'description':
					title = element.text
					
			if not missing_file:
					#shutil.copyfile(join(input_data['rom_path'], rom), join(Temp, rom))
					print ("%12s - \033[92mVerified" + '\033[0m')  % rom
		...
		#for game in root.iter(tag="game"):
			#print rom_list[game.attrib['name']]
			#for element in game:
				#if element.tag == 'rom':
					#try:
						#crc = element.attrib['crc']
					#except:
						#crc = None
		return rom_list
		...
		for index, rom in enumerate(rom_data):
			...
			if not isfile(join(rom['image_path'],rom['image_file'])) or overwrite_existing:
				...
				if 'image' in meta.getheaders("Content-Type")[0]:
					...
					print "Downloading: \033[94m%s\033[0m Bytes: %s" % (rom['image_file'], file_size)
					...
					print "No image for: \033[93m%s\033[0m" % rom['real_name']
			else:
				print "File already exists: \033[92m%s\033[0m" % rom['image_file']
